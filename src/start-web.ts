...
export async function startWeb (bot: Wechaty): Promise<void> {
  ...
  const handler = () => {
    if (qrcodeValue) {
      const html = [
        `<h1>TeamCode Wechaty Getting Started v${VERSION}</h1>`,
        'Scan QR Code: <br />',
        qrcodeValue + '<br />',
      ...
      return `TeamCode Wechaty Getting Started v${VERSION} User ${userName} logined`
    } else {
      return `TeamCode Wechaty Getting Started v${VERSION} Hello, come back later please.`
    }
  }
